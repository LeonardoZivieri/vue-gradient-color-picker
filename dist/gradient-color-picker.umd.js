(function (global, factory) {
	...
	(global = global || self, factory(global.GradientColorPicker = {}));
	...
	var script = {
		...
		methods: {
			...
			colorMove: function colorMove(event, trigger) {
				if (this.movingColor) {
					var rect = this.$refs['gradient-color-picker-input-div'].getBoundingClientRect();
					var x = event.clientX - rect.left; // x position within the element
					var percent = (100 * x) / rect.width;
			...
			registerNewValue: function registerNewValue(newValue) {
				...
				if (directionPatternResult && directionPatternResult[3]) {
					...
					this.colors = this.colors.filter(function (item, pos) { return this$1.colors.indexOf(item) === pos; });
